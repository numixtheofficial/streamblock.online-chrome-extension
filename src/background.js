 ...
 * Zuständig für:
 * 1. Statistik: Video-Ads (Stream-Swaps) + geblockte Netzwerk-Requests
 * 2. Extension-Badge
 * 3. Einstellungen (An/Aus + einzelne Methoden) verwalten
 * 4. Network-Blocking (declarativeNetRequest Ruleset) live (de)aktivieren
 */
...
const VIDEO_AD_SECONDS = 22;     // Ø Länge einer übersprungenen Video-Ad
const NET_REQUEST_SECONDS = 0.12; // Ø gesparte Ladezeit pro geblocktem Request
...
let forceBlocked = false; // gecachter Force-Update-Status (für synchrones updateBadge)
...
// Echtes Block-Log (für Detail-Ansicht)
let ruleMap = null;          // ruleId -> Domain/Label (aus rules.json)
const adLog = [];            // jüngste Video-Ad-Events aus den Content-Scripts
const AD_LOG_MAX = 80;
...
async function getBlockLog() {
  ...
  } catch (e) { /* getMatchedRules evtl. nicht verfügbar */ }
...
// Geblockte Netzwerk-Requests zählen
...
async function pollMatchedRules() {
  ...
  } catch (e) { /* Feedback-API evtl. nicht verfügbar */ }
}

function setupNetworkCounter() {
  // Dev/Unpacked: Echtzeit-Zähler
  try {
...
// Network-Blocking Ruleset live schalten
...
async function syncNetworkBlockingFromSettings() {
  // Force-Update: gesperrte Version -> Schutz hart aus, egal welche Einstellung.
  const force = await getForceState();
  if (force.blocked) {
...
// Force-Update: Badge-Warnung + Schutz abschalten
...
// Tab-Navigation: Icon bei Twitch-Tabs setzen
...
initTelemetry();
// Force-Update-Prüfung: bei Statuswechsel Schutz/Badge sofort anpassen.
