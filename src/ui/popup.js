...
(async function () {
  ...
  async function applyToActiveTab(reload) {
    ...
    // Twitch: content.js anwenden lassen (lädt bei reload selbst neu)
    if (tab.url.includes('twitch.tv')) {
      try {
        ...
        return !!res?.reloaded;
      } catch { /* Tab nicht erreichbar */ }
    }

    // YouTube: MAIN-World-Script greift erst beim Laden -> Tab neu laden
  ...
  // Anonyme Statistik (Opt-out)
  const telemetryToggle = document.getElementById('telemetryToggle');
  if (telemetryToggle) {
    chrome.storage.sync.get(['telemetry'], (s) => {
      telemetryToggle.checked = s.telemetry !== false; // default an
    });
  ...
  // Detail-Ansicht (gesperrte Vorschau in großem Fenster)
  ...
  // Gesperrte Supporter-Features -> Detail-/Spenden-Ansicht öffnen
  document.querySelectorAll('[data-open-details]').forEach((el) => {
    el.addEventListener('click', openDetails);
  });

  // Force-Update: Sperrbildschirm bei deaktivierter Version
  ...
  checkForceUpdate();
  SB_I18N.onChange(checkForceUpdate); // Texte bei Sprachwechsel aktualisieren
