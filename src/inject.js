/**
 * inject.js - Laeuft im MAIN-World der Seite (document_start).
 *
 * Kernmethode: "Stream-Swap" (Worker-Hook).
 * - Hookt window.Worker, um sich in Twitchs HLS-Worker einzuklinken (dort werden
 * die Video-Segment-Playlists geladen, NICHT im Hauptthread).
 * - Erkennt Werbung (stitched-ad) in der .m3u8 und holt einen werbefreien
 * Backup-Stream ueber einen anderen player_type (autoplay/embed/...).
 * - Der Backup-Stream ist oft nur in niedriger Aufloesung verfuegbar -> waehrend
 * der Werbung kurz schlechtere Quali, danach automatisch zurueck zur vollen Quali.
 *
 * Basiert auf TwitchAdSolutions (video-swap-new) von pixeltris (MIT):
 * https://github.com/pixeltris/TwitchAdSolutions
 * Angepasst fuer die "Streamblock" Extension (Aus-Schalter + Statistik).
 */
(function() {
 'use strict';
 ...
 window.twitchAdSolutionsVersion = ourTwitchAdSolutionsVersion;
 // [Streamblock] Aus-Schalter: bei deaktivierter Einstellung nichts hooken.
 try { if (localStorage.getItem('__tabp_disabled') === '1') { return; } } catch (e) {}
 // [Streamblock] Methoden-Einstellungen (werden von content.js in localStorage gespiegelt).
 let TABP_METHODS = { streamSwap: true, strip: true, spoof: true, network: true, dom: true };
 try {
 const __tabpRaw = localStorage.getItem('__tabp_methods');
 if (__tabpRaw) { TABP_METHODS = Object.assign(TABP_METHODS, JSON.parse(__tabpRaw)); }
 } catch (e) {}
 ...
 scope.OPT_ENABLE_STREAM_SWAP = true;
 ...
 declareOptions(self);
 IsAdStrippingEnabled = ${TABP_METHODS.strip};
 OPT_ENABLE_STREAM_SWAP = ${TABP_METHODS.streamSwap};
 OPT_FORCE_ACCESS_TOKEN_PLAYER_TYPE = ${TABP_METHODS.spoof ? "'popout'" : "''"};
 ...
 // [Streamblock] Statistik: nur die steigende Flanke (Werbung beginnt) zaehlen.
 try {
 if (e.data.hasAds && !window.__tabpHadAds) {
 window.__tabpHadAds = true;
 window.dispatchEvent(new CustomEvent('tab-ad-blocked', { detail: { type: e.data.isMidroll ? 'midroll' : 'preroll' } }));
 } else if (!e.data.hasAds) {
 window.__tabpHadAds = false;
 }
 } catch (err) {}
 ...
 // [Streamblock] Stream-Swap-Methode deaktiviert -> kein Backup-Stream holen.
 if (typeof OPT_ENABLE_STREAM_SWAP !== 'undefined' && !OPT_ENABLE_STREAM_SWAP) {
 ...
 // [Streamblock] Methoden-Flags im Fenster-Scope anwenden.
 IsAdStrippingEnabled = TABP_METHODS.strip;
 OPT_ENABLE_STREAM_SWAP = TABP_METHODS.streamSwap;
 if (!TABP_METHODS.spoof) { OPT_FORCE_ACCESS_TOKEN_PLAYER_TYPE = ''; }
 hookWindowWorker();
 hookFetch();
 monitorLiveStatus();
