/**
 * inject.js - Runs in the page's MAIN world (document_start).
 *
 * Core method: "Stream-Swap" (worker hook).
 * - Hooks window.Worker to tap into Twitch's HLS worker (where the
 * video segment playlists are loaded, NOT on the main thread).
 * - Detects ads (stitched-ad) in the .m3u8 and fetches an ad-free
 * backup stream via a different player_type (autoplay/embed/...).
 * - The backup stream is often only available in low resolution -> lower
 * quality briefly during the ad, then automatically back to full quality.
 *
 * Based on TwitchAdSolutions (video-swap-new) by pixeltris (MIT):
 * https://github.com/pixeltris/TwitchAdSolutions
 * Adapted for the "Streamblock" extension (kill switch + stats).
 */
(function() {
 'use strict';
 ...
 window.twitchAdSolutionsVersion = ourTwitchAdSolutionsVersion;
 // [Streamblock] Kill switch: don't hook anything when disabled in settings.
 try { if (localStorage.getItem('__tabp_disabled') === '1') { return; } } catch (e) {}
 // [Streamblock] Method settings (mirrored into localStorage by content.js).
 ...
 // [Streamblock] Stats: only count the rising edge (ad begins).
 try {
 ...
 // [Streamblock] Stream-Swap method disabled -> don't fetch a backup stream.
 ...
 // [Streamblock] Apply method flags in the window scope.
 IsAdStrippingEnabled = TABP_METHODS.strip;
 OPT_ENABLE_STREAM_SWAP = TABP_METHODS.streamSwap;
